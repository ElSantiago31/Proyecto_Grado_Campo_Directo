    ...
    grid.innerHTML = productos.map(product => {
        ...
        <div class="product-card" data-id="${product.id}">
            ...
            <div class="product-actions" style="margin-top: auto; padding: 15px; border-top: 1px solid #eee; display: flex; gap: 5px;">
                <button class="btn btn-favorite" onclick="toggleFavorite(this, ${product.id}, '${safeName}', '${safeImg}')" style="padding: 8px 12px; font-size: 1.2rem; background: #f8f9fa; border: 1px solid #ddd;">${heartIcon}</button>
...
function toggleFavorite(btn, productId, productName, imageUrl) {
    ...
    if (isFavorite) {
        ...
        // Agregar a favoritos
        favorites.push({ id: productId, name: productName, image: imageUrl });
        btn.textContent = '❤️';
...
async function processPurchase() {
    ...
    try {
        ...
        showNotification("Hubo un error al procesar el pedido. Verifica el stock.", "error");
    } finally {
        if (btn) {
